-- Harden tenant boundaries before Supabase migration cutover (realtime.messages block skipped; deferred to cutover per CHECKPOINT.md).
...
DROP POLICY IF EXISTS "Team-scoped users can view customer documents" ON storage.objects;
DROP POLICY IF EXISTS "Team-scoped users can upload customer documents" ON storage.objects;
DROP POLICY IF EXISTS "Team managers can update customer documents" ON storage.objects;
DROP POLICY IF EXISTS "Team managers can delete customer documents" ON storage.objects;

CREATE POLICY "Team-scoped users can view customer documents"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'customer-documents' AND public.can_read_team_or_user_storage_path(auth.uid(), name));

CREATE POLICY "Team-scoped users can upload customer documents"
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'customer-documents' AND public.can_write_team_or_user_storage_path(auth.uid(), name));

CREATE POLICY "Team managers can update customer documents"
ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'customer-documents' AND public.can_manage_team_or_user_storage_path(auth.uid(), name))
WITH CHECK (bucket_id = 'customer-documents' AND public.can_manage_team_or_user_storage_path(auth.uid(), name));
...
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'message-attachments' AND public.can_read_message_attachment_path(auth.uid(), name));

CREATE POLICY "Conversation members can upload attachments"
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'message-attachments' AND public.can_write_message_attachment_path(auth.uid(), name));
...
DROP POLICY IF EXISTS "Users can view own or team profiles" ON public.profiles;
...
USING (
  auth.uid() = id
  OR public.is_super_admin(auth.uid())
  OR public.has_shared_team_role(auth.uid(), id, ARRAY['owner','admin','manager']::public.app_role[])
);
...
DROP POLICY IF EXISTS "Users can view own or team roles" ON public.user_roles;
DROP POLICY IF EXISTS "Team admins can insert roles" ON public.user_roles;
DROP POLICY IF EXISTS "Team admins can update roles" ON public.user_roles;
DROP POLICY IF EXISTS "Team admins can delete roles" ON public.user_roles;
...
USING (
  user_id = auth.uid()
  OR public.is_super_admin(auth.uid())
  OR public.has_shared_team_role(auth.uid(), user_id, ARRAY['owner','admin']::public.app_role[])
);
...
DROP POLICY IF EXISTS "Team admins can view invitations" ON public.user_invitations;
DROP POLICY IF EXISTS "Team admins can insert invitations" ON public.user_invitations;
DROP POLICY IF EXISTS "Team admins can update invitations" ON public.user_invitations;
DROP POLICY IF EXISTS "Team admins can delete invitations" ON public.user_invitations;

CREATE POLICY "Team admins can view invitations"
ON public.user_invitations FOR SELECT TO authenticated
USING (public.is_super_admin(auth.uid()) OR public.is_team_admin(auth.uid(), team_id));

CREATE POLICY "Team admins can insert invitations"
ON public.user_invitations FOR INSERT TO authenticated
WITH CHECK (public.is_super_admin(auth.uid()) OR public.is_team_admin(auth.uid(), team_id));
...
DROP POLICY IF EXISTS "Users can delete own or team-admin messages" ON public.team_messages;
DROP POLICY IF EXISTS "Conversation creators or team admins can delete" ON public.team_conversations;
DROP POLICY IF EXISTS "Conversation members can be removed by scoped admins" ON public.conversation_members;
DROP POLICY IF EXISTS "Users can delete own or team comments" ON public.entity_comments;
...
USING (
  ...
  OR EXISTS (
    SELECT 1 FROM public.conversation_members actor
...
USING (
  auth.uid() = user_id
  OR public.is_super_admin(auth.uid())
  OR (entity_type = 'booking' AND EXISTS (SELECT 1 FROM public.bookings b WHERE b.id = entity_comments.entity_id AND b.team_id IS NOT NULL AND public.is_team_admin(auth.uid(), b.team_id)))
  OR (entity_type = 'vehicle' AND EXISTS (SELECT 1 FROM public.vehicles v WHERE v.id = entity_comments.entity_id AND v.team_id IS NOT NULL AND public.is_team_admin(auth.uid(), v.team_id)))
  OR (entity_type = 'customer' AND EXISTS (SELECT 1 FROM public.customers c WHERE c.id = entity_comments.entity_id AND c.team_id IS NOT NULL AND public.is_team_admin(auth.uid(), c.team_id)))
  OR (entity_type = 'payment' AND EXISTS (SELECT 1 FROM public.payments p WHERE p.id = entity_comments.entity_id AND p.team_id IS NOT NULL AND public.is_team_admin(auth.uid(), p.team_id)))
  OR (entity_type = 'damage_claim' AND EXISTS (SELECT 1 FROM public.damage_claims d WHERE d.id = entity_comments.entity_id AND d.team_id IS NOT NULL AND public.is_team_admin(auth.uid(), d.team_id)))
);
...
DROP POLICY IF EXISTS "Team admins can view audit logs" ON public.role_audit_log;
DROP POLICY IF EXISTS "Team admins can insert audit logs" ON public.role_audit_log;
...
WITH CHECK (
  public.is_super_admin(auth.uid())
  OR (changed_by = auth.uid() AND team_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM public.team_members tm
    WHERE tm.team_id = role_audit_log.team_id
      AND tm.user_id = auth.uid()
      AND tm.is_active = true
...
DROP POLICY IF EXISTS "Users can view own or team activity" ON public.user_activity_log;
...
-- NOTE: realtime.messages ALTER TABLE and policies intentionally skipped.
-- Lovable Cloud's migration runner lacks ownership on realtime.messages.
-- To be applied at Supabase self-managed cutover (see CHECKPOINT.md).
