DROP FUNCTION IF EXISTS public.get_super_admin_tenant_health();

CREATE FUNCTION public.get_super_admin_tenant_health()
RETURNS TABLE (
  team_id uuid,
  team_name text,
  city text,
...
BEGIN
  ...
  SELECT
    ...
    GREATEST(
      (SELECT max(ual.created_at) FROM public.user_activity_log ual
         WHERE ual.team_id = t.id),
