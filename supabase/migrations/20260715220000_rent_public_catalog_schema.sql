-- M3 (renter app public read plumbing) part 1: catalog schema.
-- Ref: docs/rent/RENTER_APP_GOAL.md milestone M3; exotiq-rent roadmap §3 tasks A+B.
-- Additive only: no existing operator flows change. Defaults keep everything
-- hidden from the marketplace until explicitly opted in.

-- ---------------------------------------------------------------------
-- Task A: vehicle slugs (unique per team, deterministic backfill)
-- ---------------------------------------------------------------------
...
-- Deterministic backfill: year-make-model, suffixed -2, -3... per team on collision.
WITH base AS (
  SELECT
    v.id,
...
-- Auto-slug new vehicles so the operator UI needs no changes.
...
-- ---------------------------------------------------------------------
-- Task B: marketplace visibility (opt-in, demo/deleted teams excluded)
-- ---------------------------------------------------------------------
...
-- Central eligibility rule. Every public RPC must go through this.
