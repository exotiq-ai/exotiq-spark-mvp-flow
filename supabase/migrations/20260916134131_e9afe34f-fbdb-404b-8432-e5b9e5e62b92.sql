...
BEGIN
  ...
  SELECT
    ...
    -- published: eligible AND already switched on for the marketplace
    count(*) FILTER (
    ...
    -- eligible: passes every per-vehicle check, published or not
    count(*) FILTER (
      WHERE (checks->>'hero_photo_set')::boolean
        AND (checks->>'rate_set')::boolean
        AND (checks->>'location_set')::boolean
        AND (checks->>'status_available')::boolean
    )
  INTO v_vehicle_checks, v_ready_vehicle_count, v_eligible_vehicle_count
  FROM scored;
  ...
  v_team_checks := jsonb_build_object(
    ...
    -- eligible, not "already published" — publishing is the action this gate unlocks
    'has_ready_vehicle',      (COALESCE(v_eligible_vehicle_count, 0) > 0)
  );
  ...
  RETURN jsonb_build_object(
    ...
    'published_vehicle_count', COALESCE(v_ready_vehicle_count, 0),
    'eligible_vehicle_count', COALESCE(v_eligible_vehicle_count, 0),
