...
export const PhotoReviewQueue = ({ vehicles }: PhotoReviewQueueProps) => {
  ...
  }, [currentPhoto, skipPhoto, goToNext]);
                ...
                <motion.img
                  ...
                  onError={(e) => {
                    console.error('Image load failed:', {
                      ...
                      originalFilename: currentPhoto.original_filename,
                    });
                    const target = e.currentTarget;
                    target.style.display = 'none';
                    // Show fallback icon
                    const parent = target.parentElement;
                    if (parent && !parent.querySelector('.photo-error-fallback')) {
                      const fallback = document.createElement('div');
                      fallback.className = 'photo-error-fallback absolute inset-0 flex items-center justify-center bg-muted';
                      fallback.innerHTML = '<div class="text-center text-muted-foreground"><svg class="h-12 w-12 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg><p class="text-sm">Image unavailable</p><p class="text-xs">Try re-uploading as JPG/PNG</p></div>';
